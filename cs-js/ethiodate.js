var formatDate, getEthiopianDate, getEthiopianDay, getEthiopianMonth, getEthiopianYear, getFerangeeDOY;
getEthiopianDate = function(date, rev) {
  var day, month, year;
  day = getEthiopianDay(date);
  month = getEthiopianMonth(date);
  year = getEthiopianYear(date);
  return formatDate(year, month, day, rev);
};
getEthiopianDay = function(date) {
  var ethiopianDOY, ethiopianYear, ferangeeDay, ferangeeYear;
...
getEthiopianYear = function(date) {
  var ethiopianYear, ferangeeDay, ferangeeYear;
  ferangeeYear = date.getFullYear();
  ferangeeDay = getFerangeeDOY(date);
  if (ferangeeYear % 4 === 0) {
    if (ferangeeDay < 255) {
...
formatDate = function(year, month, day, rev) {
  if (rev == null) {
    rev = false;
  }
  if (month < 10) {
...
getFerangeeDOY = function(dateIn) {
  return Math.floor((dateIn - new Date(dateIn.getFullYear() - 1 + "-12-31")) / (24 * 60 * 60 * 1000));
};
/* Test it all out */
console.clear();
